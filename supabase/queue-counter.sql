...
create table if not exists public.queue_counters (
  ...
  updated_by uuid,
  primary key (school_id, outlet_name, counter_name)
);
...
begin
  ...
  insert into public.queue_counters (school_id, outlet_name, counter_name, current_order_id, current_queue_number, updated_at, updated_by)
  values (p_school_id, coalesce(p_outlet_name, ''), coalesce(p_counter_name, 'main'), next_order.id, next_order.queue_number, now(), p_called_by)
  on conflict (school_id, outlet_name, counter_name) do update set
...
begin
  insert into public.queue_counters (school_id, outlet_name, counter_name, current_order_id, current_queue_number, updated_at)
  values (p_school_id, coalesce(p_outlet_name, ''), coalesce(p_counter_name, 'main'), null, null, now())
  on conflict (school_id, outlet_name, counter_name) do update set
